...
class Solution:
        ...
        while True:
            ...
            for robot in self.robots:
                robot.position = Coordinates(
                    x=(robot.position.x + robot.velocity.x) % self.width,
                    y=(robot.position.y + robot.velocity.y) % self.height,
                )
            if self._has_christmas_tree():
                self._draw_board()
                return i

    def _has_christmas_tree(self) -> bool:
        coordinate_map = {(robot.position.x, robot.position.y) for robot in self.robots}
        ...
        for robot in self.robots:
            tree_coordinates = {
                ...
                (robot.position.x + 1, robot.position.y + 1),
                (robot.position.x, robot.position.y + 2),
                (robot.position.x - 2, robot.position.y + 2),
                (robot.position.x - 1, robot.position.y + 2),
                (robot.position.x, robot.position.y + 2),
                (robot.position.x + 1, robot.position.y + 2),
                (robot.position.x + 2, robot.position.y + 2),
                (robot.position.x - 3, robot.position.y + 3),
                (robot.position.x - 2, robot.position.y + 3),
                (robot.position.x - 1, robot.position.y + 3),
                (robot.position.x, robot.position.y + 3),
                (robot.position.x + 1, robot.position.y + 3),
                (robot.position.x + 2, robot.position.y + 3),
                (robot.position.x + 3, robot.position.y + 3),
            }
            for coordinate in tree_coordinates:
                if coordinate not in coordinate_map:
                    break
            else:
                return True
    ...
    def _draw_board(self):
        ...
        if result in self.previous_maps:
            raise RuntimeError()
        self.previous_maps.add(result)
        print(result)
